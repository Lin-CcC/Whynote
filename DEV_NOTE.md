...
### 32. question block 动作面只在 question 本体选中时展开

- active question block 继续按“当前选中节点向上追溯到最近的 `question`”确定，用于阅读链条重组、当前 block 判断和自动展开逻辑；但 block 级动作面本身不再对整个 active block 常驻展开。
- `直接回答当前问题 / 插入回答 / 生成追问 / 插入追问 / 生成总结 / 插入总结` 这类 question block 动作，只在该 `question` 本体被选中时显示，避免用户已经把焦点切到子节点后，父问题下面还挂着一整块大动作栏。
- 当用户选中 `answer / judgment / 答案解析 / 手写总结 / 总结检查结果` 这类内容节点时，和当前内容直接相关的 `生成追问 / 插入追问 / 生成总结 / 插入总结 / 继续修改 / 删除` 应转移到该节点自己的动作面，而不是继续占用父问题的 block 动作区。
...
- 主视图动作展示默认分两层：`question` 本体负责 block 级动作，内容节点卡片负责“通用推进动作 + 通用节点动作 + 节点专属动作”；避免把焦点已经离开的父问题动作栏继续常驻在中间视图里。

### 36. `plan-step` 折叠态默认收口为紧凑摘要头

- `plan-step` 折叠后只保留轻量摘要头：`步骤` 标签、步骤标题、当前状态、极短折叠提示和 `展开步骤` 入口。
- 折叠态不再继续显示正文 textarea、子节点区、系统判断长句、编辑态提示或其他长说明块；这些信息只属于展开态。
- 步骤折叠仍然只是 workspace 级本地视图收纳，不改变 `plan-step` 的状态语义、持久化结构或外部导航自动展开规则。

### 37. 支持正文折叠的内容型学习节点统一进入紧凑摘要态

- 只要某个学习内容节点已经接入 `collapsedNodeBodyIds`，收起正文后就应进入统一的紧凑摘要态，而不是继续保留一张重正文卡片。
- 这套折叠摘要态默认只保留：类型标签、显示标题或未命名回退、必要语义 badge、极短折叠提示、必要时的一条轻量关系提示，以及统一的 `展开正文` 入口。
