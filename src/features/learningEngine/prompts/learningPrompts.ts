  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        'judgment 要写成站在用户视角的反馈，不只是抽象诊断。它只负责四件事：1. 先接住用户这次答对了什么、抓住了什么；2. 点出还没答到的 1-3 个关键点；3. 说明这些点不补会卡在哪里、会带来什么理解问题；4. 给出下一步应该往哪个方向继续想。',
        '优先把 judgment 写成结构化内容：strengths 填“这次答得好的地方”，gaps 返回 1-3 条缺口数组，impact 说明这些缺口不补会带来什么问题，nextStep 说明接下来该往哪里想；如果只写 judgment.content，也必须按“这次答得好的地方 / 还没答到的关键点 / 不补上会卡在哪里 / 接下来可以往哪想”四段来写。',
        '不要把完整讲解、标准答案或大段纠错塞进 judgment。',
        'hint 必须单独返回。它不是 judgment 的改写，也不是答案解析的缩写，而是围绕当前缺口的微型铺垫。',
        'hint 要承接 judgment 里已经指出的“会卡在哪里 / 接下来往哪想”，补的是用户继续修改回答前缺的那层最小知识背景。',
        ...
        'hint 不要直接给完整标准答案，不要把 judgment 里的夸奖、诊断、卡点或下一步建议换个格式重复，也不要摘抄 summary 里的标准理解句子。',
        ...
        buildTeachingCitationInstruction(),
        `返回格式：{"isAnswerSufficient":true,"judgment":{"title":"","strengths":"","gaps":[""],"impact":"","nextStep":"","content":"","citations":[${buildCitationJsonShape()}]},"hint":{"focus":"","background":"","thinkingQuestion":"","content":""},"summary":{"title":"","content":"","citations":[${buildCitationJsonShape()}]},"followUpQuestions":[{"title":"","content":"","citations":[${buildCitationJsonShape()}]}]}`,
      ]
        .filter(Boolean)
        .join('\n'),
    },
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        '先读 judgment 里“还没答到的关键点 / 不补上会卡在哪里 / 接下来可以往哪想”。这条 hint 要承接这些信息，但不能把这些句子原样复述。',
        '这条 hint 要和 judgment / 答案解析相辅相成：judgment 负责帮用户看清自己答到了什么、缺了什么、会卡在哪里；hint 负责补继续思考所需的最小知识背景。',
